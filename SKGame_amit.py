...
Now for those who dont know about tkinter ,that is a standard python interface to
tk gui tool ,basically it comes by default with the python installed on your computer
be it windows pc,unix ,linux or mac

Running python -m tkinter from the command line opens wqindows demonstrarting the workinjg
of tkinter or tk and showing you the version of tkinter installed on your system
...
##############################################################
                ...
                ' Snake Game - by Amit K Verma ' 

##############################################################

Name : Amit K Verma
Operating system (made on): Kali - Linux (Dont judge im not a hacker ^_^)
IDE USED : Pycharm

Note : To run this execute this file in any IDE
...
class main(Tkinter.Tk):
	...
	def connecting_head_with_keys(self, event=None):
		...
		if key=='Left':
			...
			pass
		return
	# Creating Turning Left Function
	def turn_left(self):
		self.x=-SNAKE_MOVING_SPEED
		self.y=0
		return
	# Creating Turning Right Function
	def turn_right(self):
		self.x=SNAKE_MOVING_SPEED
		self.y=0
		return
	# Creating Turngng Up Function
	def turn_up(self):
		self.x=0
		self.y=-SNAKE_MOVING_SPEED
		return
	# Creating tURING down Function
	def turn_down(self):
		self.x=0
		self.y=SNAKE_MOVING_SPEED
		return
	...
	# Game Loss
	def game_loss(self):
		self.board.create_text(PLAYGROUND_WIDTH/2,PLAYGROUND_HEIGHT/2,text="GAME OVER..record sent to amit"\
...
if __name__ == '__main__':
 	root=main(className=" Snake Game..Made by Amit ")
 	while True:
 		root.update()
